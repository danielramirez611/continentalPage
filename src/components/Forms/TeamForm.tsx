...
const TeamForm: React.FC<TeamFormProps> = ({ project, setProject }) => {
 ...
 const handleAddOrEditMember = async () => {
  ...
  try {
    if (editingIndex !== null) {
      const memberToUpdate = {
        ...
        ...newMember,
        project_id: project.id, // ✅ Aseguramos que se envíe
      };
    ...
    console.error("❌ Error al guardar miembro:", error.response?.data || error.message);
  }
};
  ...
  const handleAddMember = async () => {
    if (!newMember || !newMember.name || !newMember.role || !newMember.bio || !newMember.avatar || !project.id) {
      alert("Por favor, completa todos los campos obligatorios.");
      return;
    }
  ...
  const updateMember = async (index: number, field: keyof TeamMember, value: string) => {
    const updatedTeam = [...project.team];
    const member = { ...updatedTeam[index], [field]: value };
  ...
  onChange={async (e) => {
    ...
    if (file) {
      ...
      if (uploadedUrl) {
        updateMember(index, "avatar", uploadedUrl);
      }
    }
  }}
  className="hidden" 
...
/>
      ...
      <motion.div
        ...
        className="bg-white rounded-xl p-6 shadow-sm border border-gray-100"
      >
        <h3 className="text-xl font-semibold mb-6 text-gray-800">Agregar Nuevo Miembro</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium mb-2 text-gray-600">Avatar</label>
            <div className="border-2 border-dashed border-gray-200 rounded-lg p-4 text-center">
